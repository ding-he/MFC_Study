...
void CClipboardDlg::OnBnClickedBtnSend()
{
	...
	{
		CString str;
		GetDlgItemText(IDC_EDIT_SEND, str);		//get text
		EmptyClipboard();						//empty the clipboard
		HGLOBAL hMem = GlobalAlloc(GMEM_MOVEABLE, str.GetLength() + 1);	//get object of memeoy block
		char *pBuf = (char*)GlobalLock(hMem);	//return the pointer of memory block
		strcpy_s(pBuf, str.GetLength() + 1, (char*)(LPCTSTR)str);		//copy data
		GlobalUnlock(hMem);
...
void CClipboardDlg::OnBnClickedBtnRecv()
{
	...
	if (OpenClipboard())
	{
		...
		{
			HANDLE hClip = GetClipboardData(CF_TEXT);		//get the handle
