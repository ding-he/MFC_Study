...
void CDllTestDlg::OnBnClickedBtnAdd()
{
	// TODO:  在此添加控件通知处理程序代码
	//CString str;
	//str.Format("5 + 6 = %d", add(5, 6));
	//MessageBox(str);

	HINSTANCE hInstance = LoadLibrary("Dll1.dll");
	typedef int(*ADDPROC)(int x, int y);			//定义函数指针
	//ADDPROC Add = (ADDPROC)GetProcAddress(hInstance, "add");
	ADDPROC Add = (ADDPROC)GetProcAddress(hInstance, MAKEINTRESOURCE(1));	//获取DLL中的函数add
	if (!Add)
	{
		MessageBox("获取失败");
		return;
	...
	FreeLibrary(hInstance);							//释放对DLL的引用
}
